...
#环境把任务参数全部外置到 config总路->具体任务下的config（在这里修改具体任务的所有配置），底层 env 只负责双臂控制、观测、显示、录像和安全裁剪；
#环境已经把reward 固定为底层 env 的 0，把 episode 时长和硬件终止留在底层，把“成功判断”交给上层 wrapper / classifier；这和你现在的 HIL-SERL 训练链路是匹配的
...
# 兼容旧代码：如果上层还在 import GalaxeaDualArmEnv，这里不让它立刻炸
GalaxeaDualArmEnv = GalaxeaArmEnv
