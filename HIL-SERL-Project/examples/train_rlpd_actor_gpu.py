    ...
    try:
        for step in pbar:
            ...
            with timer.context("step_env"):
                ...
                stored_actions = np.asarray(stored_actions, dtype=np.float32).copy()

                if stored_actions.shape == env.action_space.shape:
                    low = np.asarray(env.action_space.low, dtype=np.float32)
                    high = np.asarray(env.action_space.high, dtype=np.float32)

                    # 前 6 维必须归一化到 [-1, 1]
                    stored_actions[:6] = np.clip(stored_actions[:6], low[:6], high[:6])

                    # 第 7 维后面会被 rewrite 成 -1/0/+1，这里先 clip 也没问题
                    stored_actions[6] = np.clip(stored_actions[6], low[6], high[6])
                
                else:
                    print(
                         f"⚠️ stored_actions shape 异常，跳过 clip: "
                         f"stored_actions.shape={stored_actions.shape}, "
                         f"env.action_space.shape={env.action_space.shape}"
                    )
